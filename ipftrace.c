...
usage(void)
{
  fprintf(stderr,
      ...
      " -f, --debug-format  [DEBUG-FORMAT]    Read the debug information with specified format\n"
      "   , --mark-offset   [OFFSET]          Specify the offset of the `mark` field of sk_buff manually\n"
      "   , --vmlinux-path  [PATH]            Specify the vmlinux path manually\n"
      "   , --modules-path  [PATH]            Specify the modules path manually\n"
      "\n"
      "LEVEL        := { 0, 1, 2, 3, 4 }\n"
      "MARK         := hex number\n"
...
static bool
opt_validate(struct ipft_opt *opt)
{
...
main(int argc, char **argv)
{
  ...
  while ((c = getopt_long(argc, argv, "v:m:f:0",
          options, &optind)) != -1) {
    switch (c) {
    case 'v':
